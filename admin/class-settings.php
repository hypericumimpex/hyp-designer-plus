			...
			<div data-id="plus-options">
				...
				<table class="form-table radykal-settings-form">
					<tbody>
						<tr valign="top">
							...
							<td>
								...
								<option value=""><?php _e( 'Use Option From Main Settings', 'radykal' ); ?></option>
									<?php
										$cs_placements = self::get_color_selection_placements();
										foreach($cs_placements as $key => $value) {
						...
						<tr valign="top">
							<th scope="row"><label><?php _e('Color Selection Display Type', 'radykal'); ?></label></th>
							<td>
								<select name="plus_color_selection_display_type" class="radykal-select2" style="width: 100%;">
								<option value=""><?php _e( 'Use Option From Main Settings', 'radykal' ); ?></option>
									<?php
										$cs_displays = self::get_color_selection_displays();
										foreach($cs_displays as $key => $value) {
											echo '<option value="'.$key.'">'.$value.'</option>';
										}
									?>
								</select>
						...
						<tr valign="top">
							...
							<td>
								...
								<option value=""><?php _e( 'Use Option From Main Settings', 'radykal' ); ?></option>
									<?php
										$bulk_add_placements = self::get_bulk_add_form_placements();
										foreach($bulk_add_placements as $key => $value) {
											echo '<option value="'.$key.'">'.$value.'</option>';
										}
									?>
								</select>
			...
			return apply_filters('fpd_plus_settings', array(
				...
				'tools' => array(
					...
					array(
						...
						'description' 	=> __( 'Display the color selection items in a grid or dropdown.', 'radykal' ),
