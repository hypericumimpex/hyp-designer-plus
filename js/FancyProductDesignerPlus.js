		...
		if(FPDUtil.elementHasColorSelection(element)) {
			...
			if(element.type == FPDPathGroupName && element.getObjects().length > 1) {  //path-groups
				...
				for(var i=0; i < availableColors.length; ++i) {
					...
					$item.find('.fpd-colors').append('<div style="background-color: '+color+'" class="fpd-item fpd-tooltip" title="'+tooltipTitle+'"><div class="fpd-label">'+(dropdownActive ? tooltipTitle : color)+'</div></div>');
